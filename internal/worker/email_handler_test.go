...
func TestEmailSendHandler_ProcessTask_Success(t *testing.T) {
	...
	h := NewEmailSendHandler(emailRepo, eventRepo, domainRepo, suppressionRepo, sender, webhookDispatch, newDiscardLogger())
...
func TestEmailSendHandler_ProcessTask_CancelledEmail(t *testing.T) {
	...
	h := NewEmailSendHandler(emailRepo, eventRepo, domainRepo, suppressionRepo, sender, nil, newDiscardLogger())
